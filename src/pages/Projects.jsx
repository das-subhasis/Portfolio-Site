...
const Projects = ({projectsRef}) => {
  ...
  return (
    <div ref={projectsRef} className='text-white w-full mt-24 py-2 flex flex-col'>
      ...
      <div className='mt-20 max-w-screen-xl mx-auto flex flex-wrap gap-10 items-center justify-center'>
        {Projects.map(project => {
          return <div key={project.id} className='w-2/5 h-fit flex flex-col gap-5 bg-white text-black px-12 py-8 rounded-2xl font-dm'>
            ...
            <div className='flex flex-wrap gap-5'>
              {
                project.stack.map(
                  (tech, id) => {
                    return <div key={id} className='text-lg font-fira hover:text-[#8000ff] hover:underline hover:underline-offset-4 hover:translate-x-1 transition-all duration-200 ease-out'>{tech}</div>
                  }
                )
              }
            </div>
